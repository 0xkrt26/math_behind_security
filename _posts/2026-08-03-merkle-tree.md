...
Here is what the resulting tree looks like:

<div class="mermaid">
graph TD
    R["H(1,8) = Root R"] --> H14["H(1,4)"]
    R --> H58["H(5,8)"]
    H14 --> H12["H(1,2)"]
    H14 --> H34["H(3,4)"]
    H58 --> H56["H(5,6)"]
...
**Bob:**
*(takes $H(1, 8, Y_{1,8}) = R$ from the contract)*
...
\text{where:}\quad 2j &= \text{total keys per message } (j \text{ private keys } x_j \text{ and } j \text{ public keys } y_j) \\
s &= \text{length of one key (100 bits)}
\end{aligned}
$$
...
Here's the updated tree — greyed-out nodes have been deleted:
...
graph TD
    ...
    style H11 fill:#ccc,stroke:#999,color:#999
    style H22 fill:#ccc,stroke:#999,color:#999
    style H34 fill:#ccc,stroke:#999,color:#999
    style H44 fill:#ccc,stroke:#999,color:#999
</div>
